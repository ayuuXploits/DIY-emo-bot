...
void drawClockDisplay() {
  unsigned long elapsed = millis() - clockStartTime;
  
  // Calculate time (starts from 12:00:00)
  unsigned long totalSeconds = (elapsed / 1000) % 86400;  // Reset every 24 hours
  ...
  // Animated clock icon
  unsigned long pulse = elapsed % 1000;
  int pulseSize = 2 + (pulse / 200);
  
  display.drawCircle(10, 10, 8, SSD1306_WHITE);
  display.drawCircle(10, 10, 7, SSD1306_WHITE);
  
  // Clock hands
  display.drawLine(10, 10, 10, 5, SSD1306_WHITE);  // Hour hand
  display.drawLine(10, 10, 13, 10, SSD1306_WHITE);  // Minute hand
  display.fillCircle(10, 10, 1, SSD1306_WHITE);  // Center dot
  
  // Pulsing indicator
  for (int i = 0; i < pulseSize; i++) {
    display.drawCircle(10, 10, 10 + i, SSD1306_WHITE);
  }
}
